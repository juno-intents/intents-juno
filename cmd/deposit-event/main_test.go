...
func TestParseDepositWitnessItem(t *testing.T) {
	t.Parallel()
	...
	cm, leafIndex, err := parseDepositWitnessItem(witness)
	if err != nil {
		t.Fatalf("parseDepositWitnessItem: %v", err)
	}
	if leafIndex != 42 {
...
func TestBuildDepositEventPayload(t *testing.T) {
	t.Parallel()
