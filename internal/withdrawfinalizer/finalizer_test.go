...
type refreshingWitnessExtractor struct {
	recordingWitnessExtractor
	refreshCalls      int
	lastRefreshAnchor int64
	lastRefreshItem   []byte
	refreshRoot       common.Hash
	refreshedWitness  []byte
	refreshErr        error
}

func (e *refreshingWitnessExtractor) RefreshWithdrawWitness(_ context.Context, anchorHeight int64, witnessItem []byte) (common.Hash, []byte, error) {
	e.refreshCalls++
	e.lastRefreshAnchor = anchorHeight
	e.lastRefreshItem = append([]byte(nil), witnessItem...)
	if e.refreshErr != nil {
		return common.Hash{}, nil, e.refreshErr
	}
	return e.refreshRoot, append([]byte(nil), e.refreshedWitness...), nil
}
...
func TestFinalizer_TickReturnsDecodedRevertReason(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "bridge paused") {
...
func TestFinalizer_FallsBackToRefreshedPayloadWitnessWhenExtractorNoteMissing(t *testing.T) {
	t.Parallel()
	...
	payloadWitness := bytes.Repeat([]byte{0x66}, proverinput.WithdrawWitnessItemLen)
	refreshedWitness := bytes.Repeat([]byte{0x88}, proverinput.WithdrawWitnessItemLen)
	w := withdraw.Withdrawal{
		ID:               seq32(0x00),
		Amount:           1000,
		FeeBps:           50,
	...
	prover := &staticProofRequester{res: proofclient.Result{Seal: []byte{0x99}}}
	extractor := &refreshingWitnessExtractor{
		recordingWitnessExtractor: recordingWitnessExtractor{
			err: witnessextract.ErrNoteNotFound,
		},
		refreshRoot:      cp.FinalOrchardRoot,
		refreshedWitness: refreshedWitness,
	}
	...
	if extractor.refreshCalls != 1 {
		t.Fatalf("refresh calls: got %d want 1", extractor.refreshCalls)
	}
	if got, want := extractor.lastRefreshAnchor, int64(cp.Height); got != want {
		t.Fatalf("refresh anchor: got %d want %d", got, want)
	}
