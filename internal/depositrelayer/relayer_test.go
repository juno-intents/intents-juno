...
type scriptedProofStoreStep struct {
	rec proof.JobRecord
	err error
}

type scriptedProofStore struct {
	calls int
	plan  []scriptedProofStoreStep
}

func (s *scriptedProofStore) GetJob(_ context.Context, _ common.Hash) (proof.JobRecord, error) {
	s.calls++
	idx := s.calls - 1
	if idx < len(s.plan) {
		step := s.plan[idx]
		if step.err != nil {
			return proof.JobRecord{}, step.err
		}
		return step.rec, nil
	}
	if len(s.plan) == 0 {
	...
	last := s.plan[len(s.plan)-1]
	if last.err != nil {
		return proof.JobRecord{}, last.err
	}
	return last.rec, nil
}
...
func TestRelayer_DoesNotExhaustProofAttemptsOnProofStoreLookupError(t *testing.T) {
	t.Parallel()
	...
	if err == nil {
		t.Fatal("expected proof store lookup error")
	}
	if !strings.Contains(err.Error(), "load proof job") {
		t.Fatalf("expected proof store lookup failure, got %v", err)
	}
	...
	if proofStore.calls != 2 {
		t.Fatalf("proof store calls: got %d want 2", proofStore.calls)
	}
	if prover.calls != 0 {
	...
	depositIDBytes, err := idempotency.DepositIDV1([32]byte(cm), 11)
	if err != nil {
		t.Fatalf("DepositIDV1: %v", err)
	}
	job, err := store.Get(ctx, depositIDBytes)
...
func TestRelayer_FlushDue_PromotesSeenWithoutCheckpoint(t *testing.T) {
	t.Parallel()

	store := deposit.NewMemoryStore()
	depID := seq32ForRelayer(0x51)
	dep := deposit.Deposit{
		DepositID:     depID,
	...
	if err := r.FlushDue(context.Background()); err != nil {
...
func TestRelayer_FlushContinuesPastRecoveredBatchResubmitError(t *testing.T) {
	t.Parallel()
	...
	cp := checkpoint.Checkpoint{
		Height:           123,
		BlockHash:        seq32ForRelayer(0xd1),
		FinalOrchardRoot: seq32ForRelayer(0xd2),
		BaseChainID:      uint64(baseChainID),
		BridgeContract:   bridge,
	}
	operatorAddrs, checkpointSigs := mustSignedCheckpoint(t, cp)
	...
	staleDep := deposit.Deposit{
		DepositID:        seq32ForRelayer(0xd3),
		Commitment:       seq32ForRelayer(0xd4),
		LeafIndex:        1,
		Amount:           500,
		BaseRecipient:    to20(common.HexToAddress("0x0000000000000000000000000000000000000456")),
		ProofWitnessItem: testDepositWitnessItem(),
	...
	staleBatchID := seq32ForRelayer(0xd5)
	if _, err := store.MarkBatchSubmitted(
		context.Background(),
		"owner-1",
		staleBatchID,
	...
	freshDep := deposit.Deposit{
		DepositID:        seq32ForRelayer(0xd6),
		Commitment:       seq32ForRelayer(0xd7),
		LeafIndex:        2,
		Amount:           700,
		BaseRecipient:    to20(common.HexToAddress("0x0000000000000000000000000000000000000789")),
		ProofWitnessItem: testDepositWitnessItem(),
	...
	if staleJob.TxHash != ([32]byte{}) {
		t.Fatalf("staleDep tx hash: got %x want empty", staleJob.TxHash)
	}
}
...
func TestRelayer_FlushDueRefreshesNewerEmittedCheckpointFromStore(t *testing.T) {
	t.Parallel()
	...
	checkpointStore := checkpoint.NewMemoryPackageStore()
	initialRec := mustEmittedCheckpointRecord(
		t,
		initialCP,
		operatorAddrs,
		initialSigs,
	...
	if err := checkpointStore.UpsertPackage(context.Background(), initialRec); err != nil {
		t.Fatalf("UpsertPackage(initial): %v", err)
	}
	...
	r, err := New(Config{
		...
		OWalletIVKBytes:   testOWalletIVKBytes(),
		TipHeightProvider: &stubTipHeightProvider{height: int64(nextCP.Height)},
		CheckpointStore:   checkpointStore,
		Now:               time.Now,
	}, deposit.NewMemoryStore(), &stubSender{}, &stubProofRequester{}, nil)
	if err != nil {
	...
	if err := r.FlushDue(ctx); err != nil {
		t.Fatalf("FlushDue(initial): %v", err)
	}
	if r.checkpoint == nil || r.checkpoint.Height != initialCP.Height {
		if r.checkpoint == nil {
			t.Fatalf("checkpoint was not restored")
		}
		t.Fatalf("restored checkpoint height: got=%d want=%d", r.checkpoint.Height, initialCP.Height)
	}

	nextRec := mustEmittedCheckpointRecord(
		t,
		nextCP,
		operatorAddrs,
		nextSigs,
	...
	if err := checkpointStore.UpsertPackage(context.Background(), nextRec); err != nil {
		t.Fatalf("UpsertPackage(next): %v", err)
	}

	if err := r.FlushDue(ctx); err != nil {
		t.Fatalf("FlushDue(refresh): %v", err)
	}
	if r.checkpoint == nil {
		t.Fatalf("checkpoint was cleared")
	}
	if r.checkpoint.Height != nextCP.Height {
		t.Fatalf("refreshed checkpoint height: got=%d want=%d", r.checkpoint.Height, nextCP.Height)
	}
	if r.checkpoint.BlockHash != nextCP.BlockHash {
		t.Fatalf("refreshed checkpoint block hash: got=%s want=%s", r.checkpoint.BlockHash.Hex(), nextCP.BlockHash.Hex())
	}
	if r.checkpoint.FinalOrchardRoot != nextCP.FinalOrchardRoot {
		t.Fatalf("refreshed checkpoint root: got=%s want=%s", r.checkpoint.FinalOrchardRoot.Hex(), nextCP.FinalOrchardRoot.Hex())
	}
}
...
func TestRelayer_DLQ_ProofAttemptsExhausted(t *testing.T) {
	...
	proofBatchID := idempotency.DepositBatchIDV1([]common.Hash{common.Hash(depositIDBytes)})
	r.proofAttempts[proofBatchID] = 1

	// Ingest deposit -> triggers submit -> proof budget is already at 1 of 2, so
	// the next proof-request path must exhaust and DLQ the batch immediately.
