...
func TestAggregator_RestoreEmittedDigestSuppressesReemit(t *testing.T) {
	t.Parallel()
	...
	if pkg, ok, err := a.AddSignature(SignatureMessageV1{
		...
		t.Fatalf("expected emitted digest to be ignored: pkg=%v ok=%v err=%v", pkg, ok, err)
	}
}
