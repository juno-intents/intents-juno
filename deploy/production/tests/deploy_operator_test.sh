...
EOF
  ...
  assert_contains "$(cat "$log_dir/ssh.stdin")" 'ensure_dkg_binary "dkg-admin" "$dkg_release_tag" "$runtime_dir/bin"' "remote deploy fetches the Linux dkg-admin release artifact"
  assert_contains "$(cat "$log_dir/ssh.stdin")" 'dkg_admin_runtime_bin="$(ensure_dkg_binary "dkg-admin" "$dkg_release_tag" "$runtime_dir/bin")"' "remote deploy records the downloaded dkg-admin binary path"
  ...
  assert_contains "$(cat "$log_dir/ssh.stdin")" 'exec /usr/local/bin/dkg-admin serve --config "$admin_config"' "remote deploy detects legacy dkg-admin wrapper path"
