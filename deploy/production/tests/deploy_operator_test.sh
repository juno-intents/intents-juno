...
EOF
  ...
  assert_contains "$(cat "$log_dir/ssh.stdin")" 'deposit_claim_ttl="${DEPOSIT_RELAYER_CLAIM_TTL:-7m}"' "remote deploy writes the deposit wrapper with a durable claim ttl"
  ...
  assert_contains "$(cat "$log_dir/ssh.stdin")" 'deposit_submit_timeout="${DEPOSIT_RELAYER_SUBMIT_TIMEOUT:-5m}"' "remote deploy writes the deposit wrapper with a bounded submit timeout"
