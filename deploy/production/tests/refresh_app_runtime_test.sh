...
test_refresh_app_runtime_bootstraps_all_in_service_app_instances_via_ssm() {
  ...
  assert_contains "$(cat "$output_dir/bin/bridge-api-wrapper")" '--bridge-paused="${BRIDGE_API_BRIDGE_PAUSED:-false}"' "refresh passes bridge pause as a bool assignment so later flags are parsed"
  assert_contains "$(cat "$output_dir/bin/bridge-api-wrapper")" '--bridge-pause-message "${BRIDGE_API_BRIDGE_PAUSE_MESSAGE:-Bridge is paused.}"' "refresh passes the bridge pause message after the bool flag"
  assert_contains "$(cat "$output_dir/bin/bridge-api-wrapper")" '--wjuno-address "${BRIDGE_API_WJUNO_ADDRESS}"' "refresh passes the optional wjuno address after the bool flag"
...
test_refresh_app_runtime_restores_kafka_ingress_for_postgres_queue_shadow() {
  local tmp fake_bin aws_log shared_manifest app_deploy secret_contract output_dir operators_root
  tmp="$(mktemp -d)"
  fake_bin="$tmp/bin"
  aws_log="$tmp/aws.log"
  ...
  app_deploy="$tmp/app/app-deploy.json"
  write_refresh_runtime_shared_manifest_fixture "$shared_manifest"
  jq '.shared_services.queue.driver = "postgres" | .shared_services.queue.shadow.driver = "kafka"' \
    "$shared_manifest" >"$shared_manifest.next"
  mv "$shared_manifest.next" "$shared_manifest"
  write_refresh_runtime_app_secret_contract "$secret_contract"
  write_refresh_runtime_app_deploy_fixture "$app_deploy" "$secret_contract"
  ...
  assert_contains "$(cat "$aws_log")" "authorize-security-group-ingress --group-id sg-shared1234567890" "refresh restores preview app ingress to shared services"
  assert_contains "$(cat "$aws_log")" '"FromPort":5432' "refresh restores shared postgres ingress for the app security group"
  assert_contains "$(cat "$aws_log")" '"FromPort":9098' "refresh restores shared kafka ingress when kafka is active as a queue shadow"
  assert_contains "$(cat "$aws_log")" "Kafka from app runtime" "refresh describes kafka ingress when a shadow queue uses kafka"
  assert_eq "$(jq -r '.ready_for_deploy' "$tmp/refresh-summary.json")" "true" "refresh summary reports success"
