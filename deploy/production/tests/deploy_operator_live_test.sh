...
EOF
  ...
  assert_contains "$(cat "$log_dir/commands.log")" "set -euo pipefail" "live deploy runs the remote rollout with strict shell options"
  assert_contains "$(cat "$log_dir/commands.log")" 'sudo bash "$remote_stage_dir/run-operator-rollout.sh"' "live deploy runs the host rollout entrypoint through bash over ssm"
