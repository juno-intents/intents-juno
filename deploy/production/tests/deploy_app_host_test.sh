...
EOF
  ...
  assert_contains "$(cat "$log_dir/ssh.stdin")" 'if [[ -n "${BACKOFFICE_OWALLET_UA:-}" ]]; then' "backoffice wrapper guards owallet ua flag"
  assert_contains "$(cat "$log_dir/ssh.stdin")" '--owallet-ua "$BACKOFFICE_OWALLET_UA"' "backoffice wrapper passes owallet ua"
