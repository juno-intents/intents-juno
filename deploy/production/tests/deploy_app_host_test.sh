...
EOF
  ...
  assert_contains "$(cat "$log_dir/ssh.stdin")" '@backoffice_vpn {' "edge caddy defines a vpn matcher for backoffice"
  assert_contains "$(cat "$log_dir/ssh.stdin")" 'remote_ip 172.27.48.0/20' "edge caddy restricts backoffice to the wireguard cidr"
  assert_contains "$(cat "$log_dir/ssh.stdin")" 'handle_path @backoffice_vpn {' "edge caddy only proxies allowed vpn backoffice traffic"
  assert_contains "$(cat "$log_dir/ssh.stdin")" 'handle_path /ops* {' "edge caddy explicitly handles disallowed backoffice traffic"
  assert_contains "$(cat "$log_dir/ssh.stdin")" 'respond "forbidden" 403' "edge caddy returns forbidden for non-vpn backoffice traffic"
