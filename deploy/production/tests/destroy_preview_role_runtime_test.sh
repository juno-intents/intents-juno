...
assert_line_order() {
  ...
  second_line="$(awk -v needle="$second" -v first_line="${first_line:-0}" 'NR > first_line && index($0, needle) { print NR; exit }' <<<"$haystack")"
...
test_destroy_preview_role_runtime_tears_down_edge_then_app_then_shared() {
  ...
  assert_contains "$(cat "$tf_log")" "terraform destroy -auto-approve -input=false -var-file=$tmp/preview/shared-terraform.auto.tfvars.json -target=aws_backup_selection.shared_postgres[0] -target=aws_backup_plan.shared_postgres[0]" "preview destroy tears down the shared backup schedule before the main shared terraform destroy"
  ...
  assert_line_order "$(cat "$aws_log")" "aws rds modify-db-cluster --profile juno --region us-east-1 --db-cluster-identifier intents-juno-shared-preview-shared-aurora --no-deletion-protection --apply-immediately" "aws backup delete-recovery-point --profile juno --region us-east-1 --backup-vault-name intents-juno-shared-preview-shared-postgres" "preview destroy disables aurora deletion protection before purging shared backup recovery points"
  assert_line_order "$(cat "$aws_log")" "terraform destroy -auto-approve -input=false -var-file=$tmp/preview/shared-terraform.auto.tfvars.json -target=aws_backup_selection.shared_postgres[0] -target=aws_backup_plan.shared_postgres[0]" "aws backup delete-recovery-point --profile juno --region us-east-1 --backup-vault-name intents-juno-shared-preview-shared-postgres" "preview destroy disables the shared backup schedule before purging vault recovery points"
...
test_destroy_preview_role_runtime_uses_live_e2e_shared_resource_names() {
  ...
  assert_contains "$(cat "$combined_log")" "aws rds modify-db-cluster --profile juno --region us-east-1 --db-cluster-identifier juno-live-e2e-preview0316d-shared-aurora --no-deletion-protection --apply-immediately" "preview destroy derives the live-e2e aurora identifier from the deployment id"
  assert_contains "$(cat "$combined_log")" "aws backup delete-recovery-point --profile juno --region us-east-1 --backup-vault-name juno-live-e2e-preview0316d-shared-postgres" "preview destroy purges the live-e2e primary backup vault"
  assert_contains "$(cat "$combined_log")" "aws backup delete-recovery-point --profile juno --region us-west-2 --backup-vault-name juno-live-e2e-preview0316d-shared-postgres-dr" "preview destroy purges the live-e2e dr backup vault"
