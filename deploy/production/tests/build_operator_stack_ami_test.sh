...
test_build_operator_stack_ami_uses_checksum_and_env_wiring() {
  ...
  assert_contains "$hydrator_script" 'base_relayer_private_keys="$(resolve_value "BASE_RELAYER_PRIVATE_KEYS"' "config hydrator resolves base relayer private keys"
  assert_contains "$hydrator_script" 'base_relayer_auth_token="$(resolve_value "BASE_RELAYER_AUTH_TOKEN"' "config hydrator resolves base relayer auth token"
  assert_contains "$hydrator_script" 'tss_auth_token="$(resolve_value "TSS_AUTH_TOKEN"' "config hydrator resolves tss auth token"
  assert_contains "$hydrator_script" 'withdraw_coordinator_juno_wallet_id="$(resolve_value "WITHDRAW_COORDINATOR_JUNO_WALLET_ID"' "config hydrator resolves withdraw coordinator wallet id"
  assert_contains "$hydrator_script" 'withdraw_finalizer_juno_scan_wallet_id="$(resolve_value "WITHDRAW_FINALIZER_JUNO_SCAN_WALLET_ID"' "config hydrator resolves withdraw finalizer wallet id"
  assert_contains "$hydrator_script" 'juno_scan_bearer_token="$(resolve_value "JUNO_SCAN_BEARER_TOKEN"' "config hydrator resolves optional juno scan bearer token"
  ...
  assert_contains "$hydrator_script" 'required_key "BASE_RELAYER_PRIVATE_KEYS" "$base_relayer_private_keys"' "config hydrator requires base relayer private keys"
  assert_contains "$hydrator_script" 'required_key "BASE_RELAYER_AUTH_TOKEN" "$base_relayer_auth_token"' "config hydrator requires base relayer auth token"
  assert_contains "$hydrator_script" 'required_key "TSS_AUTH_TOKEN" "$tss_auth_token"' "config hydrator requires tss auth token"
  assert_contains "$hydrator_script" 'required_key "WITHDRAW_COORDINATOR_JUNO_WALLET_ID" "$withdraw_coordinator_juno_wallet_id"' "config hydrator requires withdraw coordinator wallet id"
  assert_contains "$hydrator_script" 'required_key "WITHDRAW_FINALIZER_JUNO_SCAN_WALLET_ID" "$withdraw_finalizer_juno_scan_wallet_id"' "config hydrator requires withdraw finalizer wallet id"
  ...
  assert_contains "$hydrator_script" 'set_env_value "$tmp_env" BASE_RELAYER_PRIVATE_KEYS "$base_relayer_private_keys"' "config hydrator persists base relayer private keys"
  assert_contains "$hydrator_script" 'set_env_value "$tmp_env" BASE_RELAYER_AUTH_TOKEN "$base_relayer_auth_token"' "config hydrator persists base relayer auth token"
  assert_contains "$hydrator_script" 'set_env_value "$tmp_env" TSS_AUTH_TOKEN "$tss_auth_token"' "config hydrator persists tss auth token"
  assert_contains "$hydrator_script" 'set_env_value "$tmp_env" WITHDRAW_COORDINATOR_JUNO_WALLET_ID "$withdraw_coordinator_juno_wallet_id"' "config hydrator persists withdraw coordinator wallet id"
  assert_contains "$hydrator_script" 'set_env_value "$tmp_env" WITHDRAW_FINALIZER_JUNO_SCAN_WALLET_ID "$withdraw_finalizer_juno_scan_wallet_id"' "config hydrator persists withdraw finalizer wallet id"
  ...
  assert_contains "$deposit_wrapper" 'deposit_claim_ttl_seconds="$((relayer_submit_timeout_seconds + 120))"' "deposit wrapper derives claim ttl from submit timeout"
  assert_contains "$deposit_wrapper" 'deposit_claim_ttl="${DEPOSIT_RELAYER_CLAIM_TTL:-${deposit_claim_ttl_seconds}s}"' "deposit wrapper sets a derived claim ttl by default"
  ...
  assert_contains "$deposit_wrapper" 'deposit_base_rpc_url="${DEPOSIT_RELAYER_BASE_RPC_URL:-${BASE_RPC_URL:-${BASE_RELAYER_RPC_URL:-${BASE_EVENT_SCANNER_BASE_RPC_URL:-}}}}"' "deposit wrapper prefers the dedicated relayer rpc url override"
  assert_contains "$deposit_wrapper" '--max-items "${deposit_max_items}"' "deposit wrapper passes the deposit batch size override"
  assert_contains "$deposit_wrapper" '--base-rpc-url "${deposit_base_rpc_url}"' "deposit wrapper passes the dedicated base rpc url"
  assert_contains "$deposit_wrapper" '--juno-rpc-url "${deposit_juno_rpc_url}"' "deposit wrapper passes the derived juno rpc url"
  ...
  assert_contains "$aggregator_wrapper" '[[ -n "${OPERATOR_ADDRESS:-}" ]] || {' "checkpoint aggregator requires operator address in operator env"
  assert_contains "$aggregator_wrapper" 'checkpoint_aggregator_queue_group="${CHECKPOINT_AGGREGATOR_QUEUE_GROUP:-checkpoint-aggregator-${OPERATOR_ADDRESS}}"' "checkpoint aggregator derives a per-operator queue group"
...
test_build_operator_stack_ami_supports_builder_only_junocashd_reindex() {
  ...
  assert_contains "$script_text" 'sudo awk -v value="$value"' "reindex helper rewrites the protected env file through sudo-aware awk"
  assert_contains "$script_text" "' \"\$env_file\" >\"\$tmp\"" "reindex helper passes the protected env file directly to sudo awk"
  assert_not_contains "$script_text" 'sudo cat "$env_file" | awk -v value="$value"' "reindex helper no longer pipes the env file into awk"
