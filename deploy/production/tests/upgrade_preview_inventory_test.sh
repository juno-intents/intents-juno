...
test_upgrade_preview_inventory_preserves_production_shared_v2_inputs() {
  ...
  jq '
    ...
    | .app_role = {
        ...
        operator_endpoints: .app_host.operator_endpoints,
        publish_public_dns: false,
        app_instance_profile_name: "preview-app-runtime-instance-profile"
      }
    | del(.app_host)
  ' "$inventory" >"$tmp/inventory.next"
  mv "$tmp/inventory.next" "$inventory"
  ...
  assert_eq "$(jq -r '.shared_services.terraform_dir' "$output")" "deploy/shared/terraform/production-shared" "partial v2 preview upgrade preserves production-shared terraform dir when lineage is not live-e2e"
