...
EOF
  ...
  assert_contains "$(cat "$log_dir/curl.log")" "https://ops.alpha.intents-testing.thejunowallet.com/api/funds" "backoffice funds checked"
...
EOF
  ...
  assert_eq "$(cat "$log_dir/https___ops_alpha_intents_testing_thejunowallet_com_readyz.count")" "3" "backoffice ready retried twice before success"
  rm -rf "$workdir"
}
