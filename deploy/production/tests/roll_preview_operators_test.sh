...
write_fake_deploy_operator_binary_requires_force() {
  local target="$1"
  local log_file="$2"
  cat >"$target" <<EOF
#!/usr/bin/env bash
...
printf 'deploy-operator expected --force for refreshed operators\n' >&2
exit 1
EOF
  chmod +x "$target"
}
...
JSON
  ...
  write_fake_operator_release_downloader "$tmp/bin/gh" "$releases_dir" "$gh_log"
  write_fake_roll_preview_aws "$tmp/bin/aws" "$aws_log"
  write_fake_deploy_operator_binary_requires_force "$tmp/bin/deploy-operator.sh" "$deploy_log"
  write_fake_operator_canary_binary "$tmp/bin/canary-operator-boot.sh" "$canary_log"
  cat >"$tmp/bin/ssh-keyscan" <<EOF
#!/usr/bin/env bash
printf 'ssh-keyscan %s\n' "\$*" >>"$ssh_keyscan_log"
...
EOF
  ...
  assert_contains "$(cat "$deploy_log")" "--force" "preview operator roll forces redeploy after refreshing operator instances"
  assert_eq "$(jq -r '.ready_for_deploy' "$tmp/roll-summary.json")" "true" "preview operator roll succeeds when refreshed operators are force redeployed"
