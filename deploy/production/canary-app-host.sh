...
Checks:
  - App runtime instances are reachable for SSM-local checks
  - bridge-api/backoffice services are active over SSM
  - Public bridge-api and backoffice probes succeed
...
shared_manifest_path="$(production_abs_path "$(dirname "$app_deploy")" "$(production_json_required "$app_deploy" '.shared_manifest_path | select(type == "string" and length > 0)')")"
[[ -f "$shared_manifest_path" ]] || die "shared manifest not found: $shared_manifest_path"

aws_profile="$(production_json_required "$app_deploy" '.aws_profile | select(type == "string" and length > 0)')"
...
app_public_lb_target_group_arn="$(jq -r '.app_role.public_lb.target_group_arn // empty' "$app_deploy")"
app_internal_lb_target_group_arn="$(jq -r '.app_role.internal_lb.target_group_arn // empty' "$app_deploy")"
app_host="$(jq -r '.app_role.host // .app_host // empty' "$app_deploy")"
[[ -n "$app_host" || -n "$app_role_asg" ]] || die "app deploy manifest must define app_role.asg or app_host"
...
base_rpc_url="$(production_shared_manifest_base_rpc_url "$shared_manifest_path")"
...
app_capacity_detail="single-instance runtime"
public_bridge_lb_status="skipped"
public_bridge_lb_detail="single-instance runtime"
internal_backoffice_lb_status="skipped"
internal_backoffice_lb_detail="single-instance runtime"
...
min_deposit_admin_detail="backoffice minDepositAdmin matches chain state"
shared_proof_services_status="passed"
shared_proof_services_detail="shared proof runtime healthy"
...
ssm_backoffice_authenticated_get() {
  local instance_id="$1"
  local path="$2"
  local remote_cmd
...
set +a
[[ -n "\${BACKOFFICE_AUTH_SECRET:-}" ]] || { echo "missing BACKOFFICE_AUTH_SECRET" >&2; exit 1; }
...
check_target_group_health() {
  ...
  local min_healthy="$2"
  local target_health_json attempt
...
if [[ "$dry_run" == "true" ]]; then
  ...
  if [[ -n "$app_role_asg" ]]; then
    ...
    if [[ -z "$app_asg_json" ]]; then
      ...
      instance_id="$(jq -r '[.AutoScalingGroups[0].Instances[]? | select(.LifecycleState == "InService" and .HealthStatus == "Healthy")][0].InstanceId // empty' <<<"$app_asg_json")"
    fi
  ...
  if [[ "$backoffice_probe_transport" == "ssm-local" ]]; then
    if ! ssm_http_get_with_retry "$instance_id" "${backoffice_probe_url}/readyz" "backoffice readyz" >/dev/null; then
      backoffice_ready_status="failed"
      backoffice_ready_detail="backoffice /readyz failed"
    fi
    backoffice_html="$(ssm_http_get_with_retry "$instance_id" "${backoffice_probe_url}/" "backoffice html" || true)"
  else
  ...
  backoffice_settings_json="$(ssm_backoffice_authenticated_get "$instance_id" "/api/settings/runtime" 2>/dev/null || true)"
  if [[ -z "$backoffice_settings_json" ]] \
    ...
    configured_min_deposit_admin="$(jq -r '.data.minDepositAdmin // empty' <<<"$backoffice_settings_json")"
    onchain_min_deposit_admin="$(cast call --rpc-url "$base_rpc_url" "$bridge_address" "minDepositAdmin()(address)" 2>/dev/null | tr -d '[:space:]')"
    if [[ -n "$configured_min_deposit_admin" && -n "$onchain_min_deposit_admin" && "${configured_min_deposit_admin,,}" != "${onchain_min_deposit_admin,,}" ]]; then
      min_deposit_admin_status="failed"
      min_deposit_admin_detail="configured signer ${configured_min_deposit_admin} does not match on-chain minDepositAdmin ${onchain_min_deposit_admin}"
    fi
  fi
...
jq -n \
  ...
  '{
    ...
    checks: {
      ...
      app_capacity: {status: $app_capacity_status, detail: $app_capacity_detail},
      public_bridge_lb: {status: $public_bridge_lb_status, detail: $public_bridge_lb_detail},
      internal_backoffice_lb: {status: $internal_backoffice_lb_status, detail: $internal_backoffice_lb_detail},
      bridge_ready: {status: $bridge_ready_status, detail: $bridge_ready_detail},
      bridge_config: {status: $bridge_config_status, detail: $bridge_config_detail},
      deposit_memo: {status: $deposit_memo_status, detail: $deposit_memo_detail},
      bridge_frontend: {status: $bridge_frontend_status, detail: $bridge_frontend_detail},
      backoffice_ready: {status: $backoffice_ready_status, detail: $backoffice_ready_detail},
      backoffice_ui: {status: $backoffice_ui_status, detail: $backoffice_ui_detail},
      backoffice_settings: {status: $backoffice_settings_status, detail: $backoffice_settings_detail},
      shared_proof_runtime: {status: $shared_proof_runtime_status, detail: $shared_proof_runtime_detail},
      backoffice_funds: {status: $backoffice_funds_status, detail: $backoffice_funds_detail},
      min_deposit_admin: {status: $min_deposit_admin_status, detail: $min_deposit_admin_detail},
      shared_proof_services: {status: $shared_proof_services_status, detail: $shared_proof_services_detail}
    }
  }'
