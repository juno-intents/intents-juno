...
production_write_shared_terraform_override_tfvars() {
  ...
  shared_queue_driver=""
  if jq -e '(.shared_services.queue? | type == "object") and (.shared_services.queue | has("driver"))' "$inventory" >/dev/null 2>&1; then
    if ! jq -e '.shared_services.queue.driver | type == "string"' "$inventory" >/dev/null 2>&1; then
      die "shared_services.queue.driver must be kafka or postgres"
    fi
    shared_queue_driver="$(production_json_required "$inventory" '.shared_services.queue.driver')"
    shared_queue_driver="$(trim "$shared_queue_driver")"
    [[ -n "$shared_queue_driver" ]] || die "shared_services.queue.driver must be kafka or postgres"
    shared_queue_driver="$(lower "$shared_queue_driver")"
    case "$shared_queue_driver" in
      kafka | postgres) ;;
      *)
...
production_write_app_terraform_override_tfvars() {
  ...
  jq -n \
    ...
    --arg app_instance_type "$app_instance_type" \
...
production_render_operator_handoffs() {
  ...
  for ((index = 0; index < operator_count; index++)); do
    ...
    local checkpoint_blob_bucket checkpoint_blob_prefix checkpoint_blob_sse_kms_key_id deposit_relayer_release_tag juno_scan_release_tag
    ...
    juno_scan_release_tag="$(jq -r '.juno_scan_release_tag // empty' <<<"$operator_json")"
    ...
    jq -n \
      ...
      '{
        ...
        juno_scan_release_tag: (if $juno_scan_release_tag == "" then null else $juno_scan_release_tag end),
