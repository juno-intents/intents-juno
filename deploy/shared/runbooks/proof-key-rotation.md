...
No per-operator keys are used for Boundless proof funding.
...
3. Roll `proof-requestor` service:
   ...
   - verify successful offchain submissions and fallback path
4. Roll `proof-funder` service:
   - deploy with new secret reference
   - verify topup transactions are signed and mined
...
3. Requestor balance polling and topups continue.
4. No increase in auth/signing errors from Boundless operations.

## Emergency Rollback

1. Revert secret ARNs to previous known-good values.
2. Redeploy `proof-requestor` and `proof-funder`.
3. Confirm submission and topup health recovered.
