...
main() {
  ...
  assert_contains "$outputs_tf" 'value       = local.shared_msk_provisioned ? var.shared_kafka_port : null' "production-shared leaves the Kafka port output empty only when MSK is not provisioned"
  ...
  assert_contains "$variables_tf" 'variable "shared_msk_retention_mode"' "production-shared exposes MSK retention mode"
  assert_contains "$variables_tf" 'shared_msk_retention_mode must be one of auto or retain.' "production-shared validates MSK retention mode"
  ...
  assert_contains "$main_tf" 'shared_msk_retained_for_cutover              = local.shared_queue_driver == "postgres" && lower(trimspace(var.shared_msk_retention_mode)) == "retain"' "production-shared can retain MSK during postgres cutover"
  assert_contains "$main_tf" 'shared_msk_provisioned                       = local.shared_queue_uses_kafka || local.shared_msk_retained_for_cutover' "production-shared decouples MSK lifecycle from queue primary"
  ...
  assert_contains "$main_tf" 'shared_proof_queue_driver cannot be kafka when MSK is not provisioned.' "production-shared rejects kafka proof queue without MSK"
  assert_contains "$main_tf" $'for_each = local.shared_msk_provisioned ? [1] : []\n    content {\n      sid = "AllowProofRoleKafkaAccess"' "production-shared keeps proof-role kafka IAM while MSK is retained"
