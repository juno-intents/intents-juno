...
main() {
  ...
  assert_contains "$main_tf" 'wireguard_gateway_private_ip = local.wireguard_enabled ? cidrhost(data.aws_subnet.wireguard_public[0].cidr_block, 11) : ""' "live-e2e pins the wireguard gateway to a stable private ip"
