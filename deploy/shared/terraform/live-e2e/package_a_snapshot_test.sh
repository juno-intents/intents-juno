...
main() {
  ...
  assert_contains "$main_tf" 'shared_kafka_cluster_arn           = coalesce(try(aws_msk_cluster.shared[0].arn, null), "")' "live-e2e guards the shared kafka cluster arn when the MSK resource is still null"
  ...
  assert_contains "$main_tf" 'shared_kafka_bootstrap_brokers     = coalesce(try(aws_msk_cluster.shared[0].bootstrap_brokers_sasl_iam, null), "")' "live-e2e shared services consume IAM bootstrap brokers"
