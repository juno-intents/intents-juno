...
main() {
  ...
  assert_contains "$main_tf" 'shared_kafka_bootstrap_brokers     = try(aws_msk_cluster.shared[0].bootstrap_brokers_sasl_iam, "")' "live-e2e shared services consume IAM bootstrap brokers"
  ...
  assert_contains "$main_tf" 'replace(try(aws_msk_cluster.shared[0].arn, ""), ":cluster/", ":topic/")' "live-e2e topic arn prefixes tolerate disabled shared services"
  assert_contains "$main_tf" 'shared_kafka_group_arn_prefix' "live-e2e derives kafka consumer group arn prefixes for IAM auth"
  assert_contains "$main_tf" 'replace(try(aws_msk_cluster.shared[0].arn, ""), ":cluster/", ":group/")' "live-e2e group arn prefixes tolerate disabled shared services"
