...
test_aws_wrapper_wires_shared_services_into_remote_e2e() {
  ...
  assert_contains "$wrapper_script_text" "shared IPFS NLB endpoint unreachable from runner; using direct IPFS endpoint=" "aws wrapper switches to direct ipfs endpoint before long connectivity wait when nlb is unreachable"
