...
test_remote_prepare_script_waits_for_cloud_init_and_retries_apt() {
  ...
  assert_contains "$script_text" "command -v cargo-risczero >/dev/null 2>&1" "cargo-risczero presence check"
  assert_contains "$script_text" "cargo-risczero already installed at target version; skipping reinstall" "cargo-risczero install skip log"
  assert_contains "$script_text" "cargo_risczero_target_version=\"3.0.5\"" "cargo-risczero target version pin"
