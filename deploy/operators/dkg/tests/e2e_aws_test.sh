...
test_local_e2e_supports_shared_infra_validation() {
  ...
  assert_contains "$e2e_script_text" "local boundless_input_mode=\"private-input\"" "boundless input mode default"
  ...
  assert_contains "$e2e_script_text" "--boundless-guest-witness-manifest" "boundless guest witness manifest option"
  assert_contains "$e2e_script_text" "boundless_input_mode == \"guest-witness-v1\"" "guest witness mode validation"
  assert_contains "$e2e_script_text" "local guest_witness_manual_mode=\"false\"" "guest witness manual mode state"
  assert_contains "$e2e_script_text" "guest_witness_auto_generate" "guest witness auto generate summary wiring"
  assert_contains "$e2e_script_text" "\"--boundless-guest-witness-manifest\" \"\$boundless_guest_witness_manifest\"" "guest witness manifest bridge forwarding"
