...
test_aws_wrapper_wires_shared_services_into_remote_e2e() {
  ...
  assert_contains "$wrapper_script_text" "--shared-boundless-requestor-secret-arn <arn>" "shared services supports pre-existing primary boundless secret arn override"
  assert_contains "$wrapper_script_text" "--shared-boundless-requestor-secret-arn-dr <arn>" "shared services supports pre-existing dr boundless secret arn override"
  ...
  assert_contains "$wrapper_script_text" "--shared-boundless-requestor-secret-arn-dr requires --shared-boundless-requestor-secret-arn" "shared services validates primary secret arn when dr override is set"
  assert_contains "$wrapper_script_text" "--shared-boundless-requestor-secret-arn requires --shared-boundless-requestor-secret-arn-dr" "shared services validates dr secret arn when primary override is set"
  ...
  assert_contains "$wrapper_script_text" "using provided boundless requestor secret arn:" "aws wrapper accepts explicit primary boundless requestor secret arn override"
  assert_contains "$wrapper_script_text" "using provided dr boundless requestor secret arn:" "aws wrapper accepts explicit dr boundless requestor secret arn override"
  assert_contains "$wrapper_script_text" "boundless_requestor_secret_created=\"false\"" "aws wrapper tracks whether primary boundless requestor secret was created during run"
  assert_contains "$wrapper_script_text" "boundless_requestor_secret_dr_created=\"false\"" "aws wrapper tracks whether dr boundless requestor secret was created during run"
  assert_contains "$wrapper_script_text" "if [[ \"\$boundless_requestor_secret_created\" == \"true\" ]]; then" "aws wrapper only schedules primary boundless secret cleanup when created"
  assert_contains "$wrapper_script_text" "if [[ \"\$boundless_requestor_secret_dr_created\" == \"true\" ]]; then" "aws wrapper only schedules dr boundless secret cleanup when created"
  ...
  assert_contains "$wrapper_script_text" "--boundless-witness-juno-scan-urls\" \"\$witness_juno_scan_urls_csv\"" "aws wrapper forwards witness scan endpoint pool"
  assert_contains "$wrapper_script_text" "--boundless-witness-juno-rpc-urls\" \"\$witness_juno_rpc_urls_csv\"" "aws wrapper forwards witness rpc endpoint pool"
  assert_contains "$wrapper_script_text" "--boundless-witness-operator-labels\" \"\$witness_operator_labels_csv\"" "aws wrapper forwards witness operator labels"
  assert_contains "$wrapper_script_text" "--boundless-witness-quorum-threshold\" \"\$witness_quorum_threshold\"" "aws wrapper forwards witness quorum threshold"
...
test_local_e2e_supports_shared_infra_validation() {
  ...
  assert_contains "$e2e_script_text" "--boundless-witness-recipient-ua" "witness recipient UA override option"
  assert_contains "$e2e_script_text" "--boundless-witness-recipient-ufvk" "witness recipient UFVK override option"
  ...
  assert_contains "$e2e_script_text" "--boundless-witness-quorum-threshold" "witness quorum threshold option"
  ...
  assert_contains "$e2e_script_text" "--recipient-ua \"$boundless_witness_recipient_ua\"" "witness generation forwards distributed DKG recipient UA"
  assert_contains "$e2e_script_text" "--recipient-ufvk \"$boundless_witness_recipient_ufvk\"" "witness generation forwards distributed DKG recipient UFVK"
  assert_contains "$e2e_script_text" "--boundless-witness-recipient-ua and --boundless-witness-recipient-ufvk are required for guest witness extraction mode" "guest witness flow requires distributed DKG recipient identity"
  ...
  assert_contains "$e2e_script_text" "endpoint_quorum_threshold: \$boundless_witness_quorum_threshold" "summary stores witness quorum threshold"
  ...
  assert_not_contains "$e2e_script_text" $'--boundless-requestor-key-file" "$direct_cli_requestor_key_file"\n      "--boundless-deposit-owallet-ivk-hex" "$boundless_deposit_owallet_ivk_hex"' "direct-cli deploy bootstrap avoids partial manual guest witness inputs"
  assert_contains "$e2e_script_text" 'direct_cli_bridge_run_args+=("--boundless-deposit-owallet-ivk-hex" "$boundless_deposit_owallet_ivk_hex")' "direct-cli run phase injects deposit owallet ivk only with manual witness files"
  assert_contains "$e2e_script_text" 'direct_cli_bridge_run_args+=("--boundless-withdraw-owallet-ovk-hex" "$boundless_withdraw_owallet_ovk_hex")' "direct-cli run phase injects withdraw owallet ovk only with manual witness files"
...
test_aws_wrapper_derives_owallet_keys_from_distributed_ufvk() {
  ...
  assert_contains "$wrapper_script_text" "completion_juno_shielded_address" "aws wrapper captures distributed dkg completion shielded address"
  ...
  assert_contains "$wrapper_script_text" "\"--boundless-witness-recipient-ua\" \"\$completion_juno_shielded_address\"" "aws wrapper forwards distributed DKG recipient UA to local e2e witness generation"
  assert_contains "$wrapper_script_text" "\"--boundless-witness-recipient-ufvk\" \"\$DISTRIBUTED_COMPLETION_UFVK\"" "aws wrapper forwards distributed DKG UFVK to local e2e witness generation"
