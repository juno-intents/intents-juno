...
test_local_e2e_supports_shared_infra_validation() {
  ...
  assert_contains "$e2e_script_text" "--queue-driver stdio" "checkpoint aggregator consumes synthetic signatures via stdio"
  assert_contains "$e2e_script_text" "checkpoint-signatures.fifo" "checkpoint signatures streamed through fifo"
  assert_contains "$e2e_script_text" "\"\$bridge_operator_signer_bin\" sign-digest" "checkpoint signatures sourced from operator signer binary"
  assert_contains "$e2e_script_text" "--operator-endpoint \"\$operator_endpoint\"" "checkpoint signatures request each operator endpoint explicitly"
