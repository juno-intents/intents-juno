...
test_remote_operator_prepare_script_boots_full_stack_services() {
  ...
  assert_contains "$script_text" "sudo chgrp ubuntu /etc/intents-juno/junocashd.conf /etc/intents-juno/operator-stack.env /etc/intents-juno/operator-stack-hydrator.env /etc/intents-juno/operator-stack-config.json /etc/intents-juno/checkpoint-signer.key" "operator prep normalizes stack file group ownership"
  assert_contains "$script_text" "sudo chmod 0640 /etc/intents-juno/junocashd.conf /etc/intents-juno/operator-stack.env /etc/intents-juno/operator-stack-hydrator.env /etc/intents-juno/operator-stack-config.json /etc/intents-juno/checkpoint-signer.key" "operator prep normalizes stack file read permissions for ubuntu services"
