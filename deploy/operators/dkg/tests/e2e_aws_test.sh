...
test_remote_prepare_script_waits_for_cloud_init_and_retries_apt() {
  ...
  assert_not_contains "$script_text" "rustup toolchain install" "runner prepare no longer installs rust toolchain for runner-local SP1 usage"
  assert_not_contains "$script_text" "cargo +1.91.1 build --release --manifest-path zk/sp1_prover_adapter/cli/Cargo.toml" "runner prepare no longer builds SP1 adapter locally"
  assert_not_contains "$script_text" "sp1-prover-adapter" "runner prepare no longer installs SP1 adapter binary"
...
test_remote_operator_prepare_script_boots_full_stack_services() {
  ...
  assert_contains "$script_text" "if ! command -v juno-txsign >/dev/null 2>&1; then" "operator prepare bootstraps juno-txsign when AMI does not provide it"
  assert_contains "$script_text" "juno-txsign_\${juno_txsign_release_tag}_linux_\${juno_txsign_arch}.tar.gz" "operator prepare installs architecture-specific juno-txsign release asset"
  assert_contains "$script_text" "sudo install -m 0755 \"\$juno_txsign_extract_dir/juno-txsign\" /usr/local/bin/juno-txsign" "operator prepare installs juno-txsign binary into /usr/local/bin"
...
test_aws_wrapper_wires_shared_services_into_remote_e2e() {
  ...
  assert_contains "$wrapper_script_text" "copy_remote_secret_bundle() {" "aws wrapper defines remote secret bundle helper"
  ...
  assert_contains "$wrapper_script_text" "run_with_local_timeout 45 scp \"\${ssh_opts[@]}\" \"\$bundle_local\" \"\$ssh_user@\$ssh_host:\$remote_bundle_path\"" "remote secret bundle copy uses bounded scp timeout"
  assert_contains "$wrapper_script_text" "remote secret bundle copy failed attempt=\$attempt/6" "remote secret bundle copy retries transient ssh/scp failures"
  assert_contains "$wrapper_script_text" "remote secret bundle unpack failed attempt=\$attempt/6" "remote secret bundle unpack retries transient ssh failures"
  assert_contains "$wrapper_script_text" "runner_secret_bundle_entries=(" "aws wrapper stages runner secrets as a bundled transfer"
  assert_contains "$wrapper_script_text" "copy_remote_secret_bundle \\" "aws wrapper uses bundled secret staging in run path"
  ...
  assert_contains "$wrapper_script_text" "runner_ipfs_api_reachable()" "aws wrapper defines runner ipfs reachability pre-probe helper"
  ...
  assert_contains "$wrapper_script_text" "shared IPFS NLB endpoint unreachable from runner; using direct IPFS endpoint for runner probes=" "aws wrapper switches only runner probes to direct ipfs endpoint before long connectivity wait when nlb is unreachable"
  ...
  assert_contains "$wrapper_script_text" 'shared_ipfs_api_url_runner="$shared_ipfs_api_url"' "aws wrapper keeps runner ipfs probe endpoint isolated from operator checkpoint ipfs endpoint"
  assert_contains "$wrapper_script_text" 'wait_for_shared_connectivity_from_runner \' "aws wrapper runs shared connectivity checks against runner-specific ipfs endpoint"
  ...
  assert_contains "$wrapper_script_text" "\"--shared-ipfs-api-url\" \"\$shared_ipfs_api_url_runner\"" "remote shared ipfs arg uses runner-reachable endpoint"
  assert_contains "$wrapper_script_text" "\"--operator-checkpoint-ipfs-api-url\" \"\$shared_ipfs_api_url_for_operator\"" "remote operator checkpoint ipfs arg uses operator-hydrator endpoint"
  ...
  assert_contains "$wrapper_script_text" "read_operator_stack_rpc_credentials() {" "aws wrapper defines operator stack rpc credential loader"
  assert_contains "$wrapper_script_text" '\\$stack_env_file' "aws wrapper escapes stack env references inside nested ssh command"
  assert_contains "$wrapper_script_text" "loaded JUNO_RPC_USER/JUNO_RPC_PASS from operator stack env host=" "aws wrapper refreshes rpc auth from live operator stack env"
  assert_contains "$wrapper_script_text" "using staged JUNO_RPC_USER/JUNO_RPC_PASS secrets (operator stack env unavailable)" "aws wrapper logs staged rpc auth fallback when live stack env fetch fails"
...
test_aws_wrapper_supports_dr_readiness_and_distributed_relayer_runtime() {
  ...
  assert_contains "$wrapper_script_text" "run_required_aws_probe_capture()" "aws wrapper defines required aws probe helper with output capture"
  assert_contains "$wrapper_script_text" "aws required probe failed (probe=\$probe_name attempt \$attempt/3); retrying in 5s" "aws wrapper retries transient required aws probe failures"
  assert_contains "$wrapper_script_text" "aws required probe failed: \$probe_name" "aws wrapper surfaces required aws probe failures with explicit probe label"
  ...
  assert_contains "$wrapper_script_text" "\"dr-sts:GetCallerIdentity\"" "aws wrapper runs dr sts readiness check through required probe helper"
  assert_contains "$wrapper_script_text" "\"dr-ec2:DescribeAvailabilityZones\"" "aws wrapper runs dr az readiness check through required probe helper"
...
test_local_e2e_supports_shared_infra_validation() {
  ...
  assert_contains "$e2e_script_text" "--operator-checkpoint-ipfs-api-url" "operator checkpoint ipfs api option"
  ...
  assert_contains "$e2e_script_text" "--shared-ipfs-api-url is required (runner-side shared-infra checkpoint package pin/fetch verification)" "shared ipfs required message"
  assert_contains "$e2e_script_text" "--shared-ecs-cluster-arn is required (shared services own all SP1 request/auction/balance/fulfillment logic)" "shared ecs cluster is mandatory in live e2e"
  assert_contains "$e2e_script_text" "--shared-proof-requestor-service-name is required (shared services own all SP1 request/auction/balance/fulfillment logic)" "shared proof-requestor service is mandatory in live e2e"
  assert_contains "$e2e_script_text" "--shared-proof-funder-service-name is required (shared services own all SP1 request/auction/balance/fulfillment logic)" "shared proof-funder service is mandatory in live e2e"
  ...
  assert_contains "$e2e_script_text" "run_shared_infra_validation_attempt \"\$checkpoint_started_at\"" "shared checkpoint validation first enforces run-bound persisted-at window"
  ...
  assert_contains "$e2e_script_text" 'bridge_operator_signer_bin="juno-txsign"' "local e2e signer resolution defaults to juno-txsign command"
  assert_contains "$e2e_script_text" "bridge_operator_signer_env=(JUNO_TXSIGN_SIGNER_KEYS=" "local e2e signer resolution seeds signer key env from operator key files"
  ...
  assert_contains "$e2e_script_text" "shared_ecs_services_stability_reason()" "proof services ecs stability uses bounded describe-service probe"
  assert_not_contains "$e2e_script_text" "aws ecs wait services-stable" "proof services ecs stability no longer relies on long AWS waiter defaults"
  ...
  assert_contains "$e2e_script_text" "local relayer_runtime_mode=\"distributed\"" "local e2e defaults relayer runtime to distributed"
  ...
  assert_contains "$e2e_script_text" "--relayer-runtime-mode=runner is forbidden in live e2e (runner is orchestration-only)" "local e2e forbids runner relayer mode for production parity"
  ...
  assert_contains "$e2e_script_text" 'distributed relayer runtime staging freshly built relayer binaries to operator hosts' "distributed relayer runtime stages fresh relayer binaries"
  assert_contains "$e2e_script_text" 'local distributed_base_relayer_bin_path="/tmp/testnet-e2e-bin/base-relayer"' "distributed relayer runtime defines staged base-relayer binary path"
  assert_contains "$e2e_script_text" 'local distributed_deposit_relayer_bin_path="/tmp/testnet-e2e-bin/deposit-relayer"' "distributed relayer runtime defines staged deposit-relayer binary path"
  assert_contains "$e2e_script_text" 'local distributed_withdraw_coordinator_bin_path="/tmp/testnet-e2e-bin/withdraw-coordinator"' "distributed relayer runtime defines staged withdraw-coordinator binary path"
  assert_contains "$e2e_script_text" 'local distributed_withdraw_finalizer_bin_path="/tmp/testnet-e2e-bin/withdraw-finalizer"' "distributed relayer runtime defines staged withdraw-finalizer binary path"
  ...
  assert_contains "$e2e_script_text" "/v1/deposits?baseRecipient=" "live e2e polls bridge-api deposits listing for auto-detected deposit"
  assert_contains "$e2e_script_text" "go run ./cmd/withdraw-request" "live e2e submits withdraw request through CLI tool"
  assert_contains "$e2e_script_text" "/v1/status/deposit/" "live e2e polls deposit status via bridge-api"
  assert_contains "$e2e_script_text" "/v1/status/withdrawal/" "live e2e polls withdrawal status via bridge-api"
  assert_contains "$e2e_script_text" "z_sendmany" "live e2e submits real Juno shielded deposit tx"
  assert_contains "$e2e_script_text" "z_getoperationstatus" "live e2e waits for Juno deposit operation completion"
  ...
  assert_contains "$e2e_script_text" "Bridge.setParams(uint96,uint96,uint64,uint64,uint256,uint256)" "refund-after-expiry scenario temporarily configures bridge refund window"
  assert_contains "$e2e_script_text" "refund-after-expiry scenario restoring Bridge.setParams(uint96,uint96,uint64,uint64,uint256,uint256)" "refund-after-expiry scenario restores original bridge params"
  ...
  assert_contains "$e2e_script_text" "JUNO_E2E_ENABLE_DIRECT_CLI_USER_PROOF" "live e2e gates direct-cli user proof scenario behind explicit opt-in"
  assert_contains "$e2e_script_text" "skipping direct-cli user proof scenario; runner SP1 proof submission is disabled" "live e2e defaults to runner orchestration-only mode for SP1 proofs"
...
test_operator_stack_ami_runbook_builds_full_stack_and_records_blockstamp() {
  ...
  assert_contains "$runbook_text" "install_juno_txsign" "runbook installs juno-txsign during operator stack bootstrap"
  assert_contains "$runbook_text" "sudo install -m 0755 /tmp/juno-txsign /usr/local/bin/juno-txsign" "runbook installs juno-txsign binary into /usr/local/bin"
...
test_aws_wrapper_supports_proof_stage_resume_without_dkg_or_redeploy() {
  ...
  assert_contains "$wrapper_script_text" 'runner_secret_bundle_entries+=("$reuse_bridge_summary_path" "reuse-bridge-summary.json")' "aws wrapper bundles reuse bridge summary file in runner secrets when configured"
  assert_contains "$wrapper_script_text" '"$remote_repo/.ci/secrets"' "aws wrapper stages bundled secrets in runner secrets dir"
...
test_aws_wrapper_preflight_runs_required_checks() {
  ...
  assert_contains "$wrapper_script_text" "sp1 requestor balance probe is enforced by shared proof-funder service at runtime" "aws wrapper preflight delegates live balance probing to shared proof services"
  assert_not_contains "$wrapper_script_text" "sp1 balance probe returned error" "aws wrapper preflight no longer shells to runner-local sp1 adapter"
  assert_not_contains "$wrapper_script_text" "resolve_local_sp1_adapter_bin()" "aws wrapper preflight no longer resolves runner-local sp1 adapter binaries"
  assert_not_contains "$wrapper_script_text" "NETWORK_PRIVATE_KEY=" "aws wrapper preflight no longer executes local SP1 balance probe with requestor key"
