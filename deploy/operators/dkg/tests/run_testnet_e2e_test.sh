...
test_witness_extraction_reuses_existing_indexed_wallet_id() {
  ...
  assert_contains "$script_text" '"${scan_url%/}/v1/wallets/${encoded_wallet_id}/notes?limit=1000&direction=incoming"' "indexed wallet fallback scans incoming wallet notes for tx visibility using scanner-supported max page size"
...
test_checkpoint_bridge_config_updates_stack_env_runtime_keys() {
  ...
  assert_contains "$script_text" '"$operator_checkpoint_ipfs_api_url"' "checkpoint bridge config updater forwards operator checkpoint ipfs api url into remote checkpoint config update"
  ...
  assert_contains "$script_text" 'set_env_value "$tmp_env" CHECKPOINT_IPFS_API_URL "$operator_checkpoint_ipfs_api_url"' "checkpoint bridge config updater writes operator checkpoint ipfs api url into operator stack env"
  ...
  assert_contains "$script_text" 'local operator_checkpoint_ipfs_api_url="${11}"' "checkpoint bridge config updater accepts operator checkpoint ipfs api url as explicit input"
  ...
  assert_contains "$script_text" '.CHECKPOINT_IPFS_API_URL = $operator_checkpoint_ipfs_api_url' "checkpoint bridge config updater rewrites checkpoint ipfs api url in operator stack config json"
  assert_contains "$script_text" '--operator-checkpoint-ipfs-api-url <url>' "run-testnet-e2e exposes a dedicated operator checkpoint ipfs api flag"
  assert_contains "$script_text" 'local operator_checkpoint_ipfs_api_url=""' "run-testnet-e2e tracks dedicated operator checkpoint ipfs api input"
  assert_contains "$script_text" '--operator-checkpoint-ipfs-api-url)' "run-testnet-e2e parses dedicated operator checkpoint ipfs api input"
  assert_contains "$script_text" 'if [[ -z "$operator_checkpoint_ipfs_api_url" ]]; then' "run-testnet-e2e defaults operator checkpoint ipfs api url when explicit override is absent"
  assert_contains "$script_text" 'operator_checkpoint_ipfs_api_url="$shared_ipfs_api_url"' "run-testnet-e2e defaults operator checkpoint ipfs api url to shared ipfs api url"
}
...
test_shared_checkpoint_validation_retries_with_relaxed_min_persisted_at_window() {
  ...
  assert_contains "$script_text" "shared_validation_ipfs_timeout_pattern='checkpoint ipfs check: context deadline exceeded'" "shared checkpoint validation detects transient ipfs timeout signature"
  ...
  assert_contains "$script_text" "shared infra validation encountered transient checkpoint ipfs timeout; retrying once with checkpoint-min-persisted-at=" "shared checkpoint validation logs explicit transient-ipfs retry context"
  assert_contains "$script_text" 'run_shared_infra_validation_attempt "$shared_validation_retry_min_persisted_at" 2>&1 | tee -a "$shared_validation_log"' "shared checkpoint validation retries once when checkpoint ipfs call times out"
