...
test_checkpoint_bridge_config_updates_stack_env_runtime_keys() {
  ...
  assert_contains "$script_text" 'sudo sed -i "s|^  --lease-name .*\\\\$|  --lease-name \"${checkpoint_signer_lease_name}\" \\\\|g" "$checkpoint_signer_script"' "checkpoint bridge config updater rewrites existing checkpoint-signer lease-name"
  assert_contains "$script_text" "if ! grep -qE '^  --owner-id .*\\\\\\\\$' \"\$checkpoint_signer_script\"; then" "checkpoint bridge config updater validates checkpoint-signer owner-id line before inserting lease-name"
